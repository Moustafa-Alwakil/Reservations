                            ...
                            <tbody>
                                @foreach ($examfees as $examfee)
                                    <tr>
                                        ...
                                        <td>{{ ucwords($examfee->clinic->physican->name['fname_ar'] . ' ' . $examfee->clinic->physican->name['lname_ar']) }}
                                        </td>
                                        <td>{{ ucwords($examfee->clinic->physican->name['fname_en'] . ' ' . $examfee->clinic->physican->name['lname_en']) }}
                                        </td>
                                        <td>{{ $examfee->clinic->name['name_ar'] }}</td>
                                        <td>{{ $examfee->clinic->name['name_en'] }}</td>
                                        <td>
                                            @can('update')
                                                <a href="{{ route('examfees.edit', ['examfee' => $examfee->id]) }}"
                                                    class="btn btn-warning">Edit</a>
                                            @endcan
                                            @can('delete')
                                                <form method="POST" class="d-inline"
                                                    action="{{ route('examfees.destroy', ['examfee' => $examfee->id]) }}">
                                                    @csrf
                                                    @method('DELETE')
