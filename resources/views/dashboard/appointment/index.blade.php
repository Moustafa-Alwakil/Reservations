                    ...
                    <table class="datatable table table-stripped text-center">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>Canceled</td> 
                                @endif
                                <td>{{ucwords($appointment->user->name['fname'].' '.$appointment->user->name['lname'])}}</td>
                                <td>{{$appointment->clinic->name['name_ar']}}</td>
                                <td>{{$appointment->clinic->name['name_en']}}</td>
                                <td>
                                    <a href="{{route('appointments.edit',['appointment'=>$appointment->id])}}" class="btn btn-warning">Edit</a>
                                    <form method="POST" class="d-inline" action="{{route('appointments.destroy',['appointment'=>$appointment->id])}}">
                                        @csrf
                                        @method('DELETE')
