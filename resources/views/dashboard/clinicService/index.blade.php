                    ...
                    <table class="datatable table table-stripped text-center">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ucwords($clinicService->clinic->physican->department->name['name_ar'])}}</td>
                                <td>{{ucwords($clinicService->clinic->physican->department->name['name_en'])}}</td>
                                <td>{{ucwords($clinicService->clinic->physican->name['fname_ar'].' '.$clinicService->clinic->physican->name['lname_ar'])}}</td>
                                <td>{{ucwords($clinicService->clinic->physican->name['fname_en'].' '.$clinicService->clinic->physican->name['lname_en'])}}</td>
                                <td>
                                    <a href="{{route('clinicservices.edit',['clinicservice'=>$clinicService->id])}}" class="btn btn-warning">Edit</a>
                                    <form method="POST" class="d-inline" action="{{route('clinicservices.destroy',['clinicservice'=>$clinicService->id])}}">
                                        @csrf
                                        @method('DELETE')
