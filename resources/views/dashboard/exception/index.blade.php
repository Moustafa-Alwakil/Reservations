                    ...
                    <table class="datatable table table-stripped text-center">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ucwords($exception->clinic->physican->name['fname_ar'].' '.$exception->clinic->physican->name['lname_ar'])}}</td>
                                <td>{{ucwords($exception->clinic->physican->name['fname_en'].' '.$exception->clinic->physican->name['lname_en'])}}</td>
                                <td>
                                    <a href="{{route('exceptions.edit',['exception'=>$exception->id])}}" class="btn btn-warning">Edit</a>
                                    <form method="POST" class="d-inline" action="{{route('exceptions.destroy',['exception'=>$exception->id])}}">
                                        @csrf
                                        @method('DELETE')
